...
scale_fill_eu <- function(...) {
  ggplot2:::manual_scale(
    "fill",
    values = setNames(
      c("#4711EE", "#383745", "#42B81D", "#E8EE11"), levels(as.factor(data$Type)),
      ...
    )
  )
}
...
scale_fill_eu <- function(...) {
  ggplot2:::manual_scale(
    "fill",
    values = setNames(
      c("#4711EE", "#383745", "#42B81D", "#E8EE11"), levels(as.factor(df$Type)),
      ...
    )
  )
}
...
df=df %>%
  .[1:length(avg_rating), ] %>% 
  rename(Number_ep = `#`) %>%
  bind_cols(Saga = Saga[1:length(avg_rating)], Rate = avg_rating,total_votes=total_votes) |> 
  separate(Airdate, c("Year","Month")) %>%
 mutate(Year = as.numeric(Year),Type=as.factor(Type))
...
g(df %>%
    ...
    ggplot(aes(mean,Type,fill=Type)) +
...
# Destaque com menor avaliação média
df |> 
  mypdf1::pdf1_freq.tbl(Type) |> (\(x) x[3,3])() |>  round(2) |> (\(x) x*100)() |> paste0("%")  |> 
  flexdashboard::valueBox(
...
# Destaque com menor avaliação média
df |> 
  mypdf1::pdf1_freq.tbl(Type) |> (\(x) x[2,3])() |>  round(2) |> (\(x) x*100)() |> paste0("%") |>  
  flexdashboard::valueBox(
...
g(df %>%
  group_by(Saga) %>%
  summarise(mean=mean(Rate),dp=sd(Rate),votes=sum(total_votes)) %>% 
...
```{r}
 df %>% group_by(Saga) %>%summarise(mean=mean(Rate)) |>filter(mean==max(mean)) |> select(Saga) |> 
  flexdashboard::valueBox(
...
```{r}
g(df %>% 
  ggplot(aes(l1=total_votes,l2=Number_ep))+
  geom_point(aes(Saga,Rate,color = Type))+
    geom_hline(yintercept = mean(df$Rate), color="#F91414")+
    ggplot2::scale_color_manual(breaks = c(levels(as.factor(df$Type))), values = c("#4711EE", "#383745", "#42B81D", "#E8EE11"))+
    coord_flip()+
    labs(x="",y="Avaliação")+
...
###
```{r}
df |> filter(Rate==max(df$Rate)) |> select(Number_ep) |> (\(x) x[1,])() |> 
  flexdashboard::valueBox(
    caption="Greatest Rating",
    icon="ion-android-favorite"
  )
...
###
```{r}
df |> filter(Rate==min(df$Rate)) |> select(Number_ep) |> (\(x) x[1,])() |> 
  flexdashboard::valueBox(
...
*Title of `r df |> filter(Rate==min(df$Rate)) |> select(Number_ep) |> (\(x) x[1,])()`: `r df |> filter(Rate==min(df$Rate)) |> select(Title) |> (\(x) x[1,])()`  *
