...
One piece é até o momento  o anime com maior duração (`r max(df$Lançamento)-min(df$Lançamento) ` Anos), inclusive sendo mais velho que o autor vos escreve. Nesse breve ensaio veremos como a obra (anime) se comporta em relação a quantidade de episódios postados por ano, tipos de episódios, e avaliação pelos fãs.

## O Tempo passa e Luffy segue em frente 

Veja que são `r max(df$Lançamento)-min(df$Lançamento)` anos de história, e mesmo assim a quantidade de episódios postados por ano tem pouca variação:
...
## Avaliação
### Avaliação por arco
Em relação a avaliação média dos usuarios no site [Rating Graph](https://www.ratingraph.com/tv-shows/one-piece-ratings-17673/) mantem-se sempre acima de 7.5 em todos os arcos, tem-se como destaque  arco Marineford e o  Wano 
```{r rate}
g(df %>%
  group_by(Saga) %>%
...
### Avaliação dos episódios por arco
...
### Avaliação dos fillers por arco
Em termos de avaliação do fãs, já sabemos que os episódios fillers são os que a galera menos gosta, mas dentre os fillers, que são os com menor avaliação? É o que veremos nessa subseção.
...
g(df %>%
  ...
  labs(y="",x="Avaliação média dos usuarios")+
     coord_flip())
```
Você pode acessar os fillers por saga [aqui](https://alissonrp.shinyapps.io/op_beta/)
